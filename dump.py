...
class Rates:
    ...
    for exch_id, exch_name in list(
            filter(lambda x: x[1] in Exchangers.exchangers_valid_name, sorted(exchangers.items()))):
    ...
    for cy_id, cy_name in list(
            filter(lambda x: x[1].split(' ')[-1].strip('()') in Tokens.tokens_ABB, sorted(currencies.items()))):
